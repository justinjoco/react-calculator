...
function BasicCalculator() {
  ...
  const description =
    "Results of each operation are immediately calculated and displayed. As a result, multiplication/division has the same precedence as addition/subtraction.";
    ...
    switch (currOperator) {
      ...
      case "/":
        if (currNumber !== 0) {
          ...
          setError(
            "You attempted to divide by zero. Please use another number",
          );
        }
        break;
    }

    const displayStr = Number.isInteger(total)
      ? String(total)
